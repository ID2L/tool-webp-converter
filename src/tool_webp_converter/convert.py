...
def compress_image_to_webp(input_path: Path, output_dir: Path = None, quality: int = 80) -> tuple[Path, int]:
    ...
    with Image.open(input_path) as img:
        ...
        if lossy_size <= lossless_size:
            ...
            logging.info(
                f"{original_format} ({original_size} bytes) → WebP (lossy): {output_path} ({lossy_size} bytes, -{reduction_percent:.1f}%)"  # noqa: E501
            )
            ...
            logging.info(
                f"{original_format} ({original_size} bytes) → WebP (lossless): {output_path} ({lossless_size} bytes, -{reduction_percent:.1f}%)"  # noqa: E501
            )
